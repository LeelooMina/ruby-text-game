...
module Auth
  ...
  def self.login
    character_confirm = false
    while character_confirm == false
        puts "Now you gotta login!"
        puts "Enter your name:"
        name_input = gets.chomp.downcase.capitalize
        ...
        @@characters.each do |character|
            puts character['name']
            puts character['password']
            if character['name'] == name_input && character['password'] == password
                puts "Oh hey, you did it."
                character_confirm = true
                break
            end
        end
    end
    name = @@characters[0]["name"]
    load_class = @@characters[0]["class"]
    $current_character = Character.new
    $current_character.set_name(name)
    $current_character.set_class(load_class)
